...
export function getHarnessFeedbackPath(worktree) {
    const dir = getProjectDir(worktree);
    ensureDir(dir);
    return join(dir, HARNESS_FEEDBACK_FILE);
}
export function saveHarnessFeedback(worktree, title, category, body, originSessionId) {
    ...
    if (originSessionId)
        meta.push(`session: ${originSessionId}`);
    const entry = `## ${title}\n` +
        meta.map((m) => `- ${m}`).join("\n") +
        "\n\n" +
        body.trim() +
