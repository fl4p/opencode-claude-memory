...
const SECRET_KW =
  "passwords?|passwd|pwd|passphrases?|api[\\s_-]?keys?|apikeys?|" +
...
function rstripTrail(s: string): string {
  let end = s.length
...
function looksSecret(v: string): boolean {
  if (!v || v.includes("/") || "~$.".includes(v[0])) return false
...
export function scrubMemoryFields(fields: { name: string; description: string; content: string }): {
  name: string
  description: string
  content: string
  count: number
