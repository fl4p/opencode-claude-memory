...
export type ExtractionDeltaReason =
  | "no-mark"
  | "new-turns"
  | "up-to-date"
  | "branch-diverged"
...
function getExtractionStateDir(worktree: string): string {
  return join(getProjectDir(worktree), "opencode-memory", "extraction")
}

function extractionSessionKey(sourceSessionId: string | undefined): string {
  return sanitizePath(sourceSessionId?.trim() || "no-session")
}

function parseExtractionMark(raw: unknown): ExtractionMark | undefined {
  if (!raw || typeof raw !== "object") return undefined
  const value = raw as Record<string, unknown>
  if (value.schemaVersion !== 1) return undefined
  if (typeof value.userTurnCount !== "number" || !Number.isFinite(value.userTurnCount)) return undefined
  if (typeof value.messageCount !== "number" || !Number.isFinite(value.messageCount)) return undefined
  ...
  return {
    ...
    userEntryIds: Array.isArray(value.userEntryIds)
      ? value.userEntryIds.filter((item): item is string => typeof item === "string")
      : undefined,
    updatedAt: value.updatedAt,
  }
}

export function truncateExtractionText(text: string, maxChars = DEFAULT_EXTRACT_MAX_CHARS): string {
  if (text.length <= maxChars) return text
  return `${text.slice(0, maxChars)}\n[truncated ${text.length - maxChars} chars]`
}

export function buildExtractionPrompt(payload: ExtractionTranscriptPayload): string {
  return [
...
export function getExtractionMarkPath(worktree: string, sourceSessionId?: string): string {
  return join(getExtractionStateDir(worktree), "marks", `${extractionSessionKey(sourceSessionId)}.json`)
}

export function readExtractionMark(worktree: string, sourceSessionId?: string): ExtractionMark | undefined {
...
export function writeExtractionMark(worktree: string, sourceSessionId: string | undefined, mark: ExtractionMark): string {
  const path = getExtractionMarkPath(worktree, sourceSessionId)
...
export function buildExtractionMark(input: Omit<ExtractionMark, "schemaVersion" | "updatedAt">): ExtractionMark {
  return {
    schemaVersion: 1,
    ...input,
...
export function decideExtractionDelta(
  mark: ExtractionMark | undefined,
  stats: ExtractionSourceStats,
): ExtractionDeltaDecision {
...
export function getExtractionLogPath(worktree: string): string {
  return join(getExtractionStateDir(worktree), "events.jsonl")
}

export function getLatestExtractionLogPath(worktree: string): string {
  return join(getExtractionStateDir(worktree), "latest.json")
}

export function appendExtractionLog(worktree: string, event: ExtractionLogEvent): string {
...
export function readLastExtractionLog(worktree: string): ExtractionLogEvent | undefined {
  const path = getExtractionLogPath(worktree)
  if (!existsSync(path)) return undefined
  ...
  for (let index = lines.length - 1; index >= 0; index--) {
    try {
      const parsed = JSON.parse(lines[index]) as ExtractionLogEvent
...
export async function withMemoryStoreLock<T>(
  worktree: string,
  callback: () => T | Promise<T>,
  options: MemoryStoreLockOptions = {},
): Promise<T> {
